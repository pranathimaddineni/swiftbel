...
import toiletimg from '../../../public/assets/plumbing3.svg';
import smellydrainsimg from '../../../public/assets/plumbing2.svg';
import leakageimg from '../../../public/assets/plumbing1.svg';
import drainagepic from '../../../public/assets/plumbing4.svg'
...
const plumbingprobsoptions= [
    { name: 'Leaking', img: leakageimg },
    { name: 'Drainage', img: drainagepic },
    { name: 'Smelly drains', img: smellydrainsimg },
    { name: 'Running toilet', img: toiletimg },
    ]
...
const [errors, setErrors] = useState(
    ...
    const handleSubmit = async()=>{
        ...
        if(isValid()){
         setIsloading(true)
         res = await dispatch(plumbers(values,1,10));
         if(res.status===true){
            router.push({pathname:'/plumbers/getPrice',query:{...values,price:res?.data?.finalPrice,extrahour:res?.data?.secondHour}})
         }
...
return(
    <Main>
             ...
             {datecalendar === true ?
                ...
                {/* <Inputhead>Or</Inputhead>
                <Placing >
                    {
                        plumbingprobsoptions?.map((item, index) =>
                        <ServicImage id={item.name} className='cardImg' src={item.img}
                        style={{
                        border:item.name===values.typeOfCall?'2px solid #5B82B6':item.name===values.typeOfCall ?'1px solid green':null}} key={index} onClick={handlechange('typeOfCall')} />
                        )
                    }
